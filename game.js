...
let inventario = [{ nombre:"hacha", tipo: "herramienta", cantidad: 1, icono:"hacha_1" }];
...
function craftear(receta){
  ...
  guardarEnInventario(itemCraft.resultado, itemCraft.tipo, itemCraft.cantidad, itemCraft.icon)
...
function create() {
  ...
  this.btnInteractuar = this.add.image(window.innerWidth - 50, 30 + 5 * 35, "btn_hit")
  .setScrollFactor(0)
  .setInteractive()
  .setScale(2)
  .setAlpha(0.7)
  ...
  this.btnGolpear = this.add.image(window.innerWidth - 100, 50 + 5 * 35, "btn_hit")
    .setScrollFactor(0)
    .setInteractive()
    .setScale(2)
    .setAlpha(0.7)
  ...
  this.panelUserContainer = this.add.container(0,0).setDepth(10000).setScrollFactor(0);

  this.btn_panel_user = this.add.image(window.innerWidth - 50, 105, "btn_panel_user").setInteractive().setAlpha(0.7).setDepth(9999).setScrollFactor(0).setScale(0.7)
  ...
  const text_user_stats = this.add.text( width/8, 50, "Estadisticas", { fontSize: "20px", color:"#fff"}).setOrigin(0.5).setInteractive().setScrollFactor(0)
  const text_user_inv = this.add.text(width/2.5, 50, "Inventario", { fontSize: "20px", color:"#fff"}).setOrigin(0.5).setInteractive().setScrollFactor(0)
  const text_user_todo = this.add.text(width/1.5, 50, "Por Hacer", { fontSize: "20px", color:"#fff"}).setOrigin(0.5).setInteractive().setScrollFactor(0)
  const text_user_confi = this.add.text(width/1.1, 50, "Configuración", { fontSize: "20px", color:"#fff"}).setOrigin(0.5).setInteractive().setScrollFactor(0)
  ...
  const text_stats_energia = this.add.text(width/2, height/4, "Energia: " + this.energiaActual, {fontSize:"20px", color:"#fff"}).setOrigin(0.5)
  this.text_stats_energia = text_stats_energia

  this.container_user_stats = this.add.container(0,0, [panel_user_stats, text_stats_energia])
  ...
  for(let i=0;i<inventarioStock;i++){
    let inv_slot = this.add.image(50 + (i+i) * 36, height/2, "slot_image")
    .setInteractive()
    .setScale(3)
    .setDepth(10010)
    .setVisible(true);
  ...
  for(let i=0; i<inventarioStock;i++){
    let icon_slot_inv = this.add.image(50 + (i+i) * 36, height/2,"slot_image")
    .setVisible(true)
    .setScale(1.3)
    .setDepth(10000);
  ...
  for(let i=0; i<inventarioStock;i++){
    let txt_slot_inv = this.add.text(66 + (i+i) * 36,height/2, "0", {
      fontSize: "15px",
      color: "#ffffff",
      stroke: "#000000",
      strokeThickness: 4
    }).setDepth(10000).setVisible(true)
  ...
  const slot_craft_info = this.add.image(width/2 + 100, 200, "slot_image").setScale(5)
  const icon_craft_info = this.add.image(width/2 + 100, 200, "slot_image").setScale(1.5).setVisible(false)

  const titulo_craft_info = this.add.text(width/2 + 170, 150, "Item a Craftear", {
    fontSize: "20px",
      color: "#ffffff",
      stroke: "#000000",
      strokeThickness: 4
  ...
  const text_craft_info = this.add.text(width/2 + 170, 190, "Presiona lo que quieres craftear,\n te daremos informacion sobre el objeto", {
    fontSize: "12px",
      color: "#ffffff",
      stroke: "#000000",
      strokeThickness: 4
  ...
  const btn_craftear_item = this.add.text(width - 100, 200, "Craftear", {
    fontSize: "15px",
      color: "#ffffff",
      stroke: "#000000",
      strokeThickness: 4
  ...
  this.container_user_inv = this.add.container(0,0, [panel_user_inv, panel_craft_inv,panel_info_craft,slot_craft_info,icon_craft_info,titulo_craft_info,text_craft_info,btn_craftear_item,...SlotsInv, ...iconosSlotsInv, ...textosCantidadInv]);
  this.container_user_inv.setVisible(false);

  recetas.forEach((receta,i) => {
    let slotReceta = this.add.image(width/2 + (50 + (i+i) * 36), height/2, "slot_image").setScale(3)
    let iconReceta = this.add.image(width/2 + (50 + (i+i) * 36), height/2, receta.icon).setScale(1).setInteractive().setScrollFactor(0)
  ...
  this.panelUserContainer.add([panel_user_bg,text_user_stats,text_user_inv,text_user_todo,text_user_confi,this.container_user_stats,this.container_user_inv,this.container_user_todo,this.container_user_config])
...
function update() {
  ...
  if(this.btnInteractuarDown || this.teclas.Q.isDown){
    ...
    if(item.tipo == "estructura"){
        const estructura = this.physics.add.staticImage(player.x + 15, player.y, inventario[slotSeleccionado].icono).setOrigin(0.5,0.5)
        estructura.setDepth(estructura.y)
        estructura.setSize(64,64)
